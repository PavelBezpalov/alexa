module Intents
  class Calculation
    ...
    def process_request
      ...
      to_pay = Transaction.where('transaction_date > ? AND transaction_date < ?', Date.parse("01-#{start_period}-#{Date.current.year}"),
                                 Date.parse("01-#{end_period}-#{Date.current.year}").end_of_month).sum(:amount) * 0.05
