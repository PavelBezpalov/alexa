module Intents
  class Calculation
    ...
    def process_request
      ...
      to_pay = Transaction.where('start_date > ? AND end_date < ?', Data.parse("01-#{start_period.to_s}-#{Date.current.year}"),
                        Data.parse("01-#{end_period.to_s}-#{Date.current.year}").end_of_month).sum(:amount)*0.05
